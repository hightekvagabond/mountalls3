...
configure_bucket_groups() {
    ...
    if [[ -n "$specified_groups" ]]; then
        ...
        echo ""
        echo "💡 Don't worry about thinking of everything now - you can create new groups"
...
assign_buckets_to_groups() {
    ...
    for profile in "${profiles[@]}"; do
        echo ""
        print_info "🔧 Processing profile: $profile"
...
show_usage() {
    ...
    echo "  • All current buckets: assign all buckets from the current profile"
    echo "  • Pattern matching: assign buckets containing text patterns (supports '*' wildcard)"
    echo "  • Specific selection: choose UNASSIGNED buckets by numbers (supports ranges like '1-5')"
    echo "  • Create new group: create groups on-the-fly during bucket assignment"
    echo ""
    echo "PATTERN EXAMPLES:"
    echo "  '*' → matches ALL buckets in the profile (wildcard)"
    echo "  '.com' → matches domain buckets (bigvision.com, mysite.com)"
    echo "  'dev' → matches development buckets (myapp-dev, dev-staging)"
    echo "  'backup' → matches backup buckets (daily-backup, user-backups)"
    echo ""
    echo "EXAMPLES:"
...
configure_default_mount_groups() {
    ...
    echo ""
    print_info "💡 You can always override defaults with:"
...
assign_buckets_by_pattern_new_group() {
    ...
    while [[ ${#remaining_buckets[@]} -gt 0 ]]; do
        ...
        if prompt_yes_no "Save pattern rule for '$pattern' in '$new_group_name'?" "y"; then
            ...
            print_success "✨ Saved pattern rule: '$pattern' → '$new_group_name'"
        ...
        2)
            ...
            while [[ ${#remaining_buckets[@]} -gt 0 ]]; do
                ...
                if prompt_yes_no "Save pattern rule for '$pattern' in '$selected_group'?" "y"; then
                    ...
                    print_success "✨ Saved pattern rule: '$pattern' → '$selected_group'"
...
create_new_group_and_assign() {
    ...
    print_success "✅ Created new group: $new_group_name"
    echo ""
...
assign_buckets_by_pattern() {
    ...
    echo "Examples:"
    echo "  • '*' → all buckets (wildcard)"
    echo "  • Contains '.com' → websites group"
    echo "  • Starts with 'gypsy' → user-files group"
    echo "  • Contains 'backup' → backup group"
    echo ""
            ...
            case "$assignment_choice" in
                1)
                    ...
                    if add_pattern_to_group "$selected_group" "$profile" "$pattern" "$pattern_desc"; then
                        print_success "✨ Saved pattern rule: '$pattern' → '$selected_group' (profile: $profile)"
